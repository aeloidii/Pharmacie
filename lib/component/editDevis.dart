...
class editDevis extends StatefulWidget {
  String docId;
  editDevis({super.key, required this.docId});

  @override
  State<editDevis> createState() => editDevisState();
}

class editDevisState extends State<editDevis> {
...
/*generate dmnd PDF*/
  ...
  Future<void> _saveDownloadUrlToFirestore() async {
    ...
    async {
      ...
      await _generateInvoice("Devis");
      await _savePdfToFirebaseStorage("Devis");
      firebaseFiretore
          .collection(
          "devis")
          .doc(widget.docId)
      ...
      await FirebaseFirestore.instance.collection('devis').doc(widget.docId)
  ...
  Future<void> _ajouterVenteFirestore() async { // STOCKER DANS FACTURE AVEC LE MEME ID DE LA COMMANDE
    await _firestore
        .collection(
        "ventes")
        .doc(widget.docId)
    ...
    await FirebaseFirestore.instance
      ...
      if (snapshot.exists) {
        setState(() async {
          ...
          // _desigFournTextController.text= await getfieldNameFromDatabase(snapshot.data()!["id_Fourn"],"Fournisseurs","désignation");
          _referenceFourTextController.text=snapshot.data()!["id_entrC"];
          String IDF=snapshot.data()!["id_entrC"];
          await FirebaseFirestore.instance
              .collection("Clients")
              .doc(IDF)
              .get()
                                ...
                                SizedBox(
                                  ...
                                  child:  Stack(
                                    children: [_isLoading2 ? Center(
                                      ...
                                      firebaseUIButton(
                                          context, "CONFIRMER ", ()async {
                                        // await _generateInvoice();
                                        // await _print();
                                        await _createBLToFirestore();
                                        await _ajouterVenteFirestore();

                                        setState(() {
                                          stateBC="Confirmé";
                                        });
                                        ScaffoldMessenger.of(context).showSnackBar(
                                            SnackBar(content: Text("Enregistrement du BL avec succès"),backgroundColor:AppColors.them,showCloseIcon: true,));                          // _desigFournTextController.text="";
                                      }, AppColors.btn, AppColors.them),
                                      Positioned(
                                        bottom: 30,
                                        right: 10,
