...
class venteRetournee extends StatefulWidget {
  venteRetournee({super.key});

  @override
  State<venteRetournee> createState() => venteRetourneeState();
}

class venteRetourneeState extends State<venteRetournee> {
...
/*generate dmnd PDF*/
  ...
  Future<void> _addVenteRetourToFirestore() async { // STOCKER DANS FACTURE AVEC LE MEME ID DE LA COMMANDE
    await _firestore
      ...
      "prixVente":_PrixUnitaireTextController.text,
    ...
    await incrementerStock(referenceArticle,_QuantitTextController.text);
    await createHistory( id_entr,  id_user,  DateFormat('yyyy-MM-dd   HH:mm').format(DateTime.now()),  "Retour de vente d'article de réf : ${referenceArticle}, Qtte: ${_QuantitTextController.text}, TTC:  ${_SousTTCTextController.text}  ");
                ...
                child: SafeArea(
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Header('Stock', 'Vente retournée'),
                        SizedBox(
                          height: 60,
                        ),
                        Responsive.isDesktop(context)?
                        PrimaryText(
                          text: 'AJOUTER UNE VENTE RETOURNEE ',
                          size: 18,
...
//Table temporaire
                            ...
                            _isLoading ? Center(
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(content: Text("Enregistrement de vente retournée avec succès"),backgroundColor:AppColors.them,showCloseIcon: true,)
                              );
                              // Navigator.push(
                              //   context,
                              //   MaterialPageRoute(
                        ...
                        Center(
                          child: PrimaryText(
                            text: 'LISTE DES VENTES RETOURNEE DE  ${EntrpNom.toUpperCase()}', size: 18, fontWeight: FontWeight.w800, color: AppColors.them,),
                        ),
                        SizedBox(
                          height: 20,
                        ),
                          ...
                          children: [
                            SizedBox(
                              ...
                              child: StreamBuilder<QuerySnapshot>(
                                  stream: FirebaseFirestore.instance.collection('retourVente').orderBy('dateAjout', descending: true).snapshots(),
                                      ...
                                      List<DataRow>? rows = snapshot.data?.docs.where((doc) {
                                        ...
                                        return DataRow(
                                          cells: [
                                            DataCell(Row(mainAxisAlignment: MainAxisAlignment.center,
                                              children: [
                                                IconButton(
                                                    ...
                                                    onPressed: () async {
                                                      await FirebaseFirestore.instance.collection('retourVente').doc(doc.id).delete();
                                            ...
                                            DataCell(Text(routeArgs['id_retourVente'] ?? 'default value')),
