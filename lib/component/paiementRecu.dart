...
class paiementRecu extends StatefulWidget {
  paiementRecu({super.key});

  @override
  State<paiementRecu> createState() => paiementRecuState();
}

class paiementRecuState extends State<paiementRecu> {
...
/*generate dmnd PDF*/
  ...
  bool status = false;
  Future<void> _addpaiementRecuToFirestore() async { // STOCKER DANS FACTURE AVEC LE MEME ID DE LA COMMANDE
    await _firestore
        .collection(
        "paiementRecu")
        .add({
                ...
                child: SafeArea(
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Header('Clients', 'Paiement Reçu'),
                        SizedBox(
                          height: 60,
                        ),
                        Responsive.isDesktop(context)?
                        PrimaryText(
                          text: 'AJOUTER UN REÇU DE PAIEMENT  ',
                          size: 18,
                                ...
                                children: [
                                  Container(
                                    ...
                                    child: firebaseUIButton(
                                      ...
                                      await _addpaiementRecuToFirestore();
                                      ScaffoldMessenger.of(context).showSnackBar(
                                          SnackBar(content: Text("Reçu de paiement enregistré avec succès"),backgroundColor:AppColors.them,showCloseIcon: true,)
                                      );
                                      // Navigator.push(
                                      //   context,
                                      //   MaterialPageRoute(
...
//                             ),
                        ...
                        Center(
                          child: PrimaryText(
                            text: 'LISTE DES REÇUS DE PAIEMENT DE  ${EntrpNom.toUpperCase()}', size: 18, fontWeight: FontWeight.w800, color: AppColors.them,),
                        ),
                        SizedBox(
                          height: 20,
                        ),
                          ...
                          children: [
                            SizedBox(
                              ...
                              child: StreamBuilder<QuerySnapshot>(
                                  stream: FirebaseFirestore.instance.collection('paiementRecu').orderBy('dateAjout', descending: true).snapshots(),
                                            ...
                                            DataCell(Row(mainAxisAlignment: MainAxisAlignment.center,
                                              children: [
                                                ...
                                                IconButton(
                                                    ...
                                                    icon: Icon(Icons.delete, color: Colors.red),
                                                    onPressed: () async {
                                                      await FirebaseFirestore.instance.collection('paiementRecu').doc(doc.id).delete();
