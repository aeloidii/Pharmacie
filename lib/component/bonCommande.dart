...
class bCommande extends StatefulWidget {
  bCommande({super.key});

  @override
  State<bCommande> createState() => bCommandeState();
}

class bCommandeState extends State<bCommande> {
...
/*generate dmnd PDF*/
  ...
  Future<void> _generateInvoice() async {
    ...
    for (var item in listOfListsDesArticles.reversed){
      List<String> _list=[item[8],item[0],_DateDeCommandeTextController.text=="" ? DateFormat('yyyy-MM-dd   HH:mm').format(DateTime.now()).substring(0,10) : _DateDeCommandeTextController.text.substring(0,10),
        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              ...
              pw.Text("Bon de Commande N° $id_commande",style: pw.TextStyle(fontSize: 16),),
              // Add the table of products to the PDF
              pw.SizedBox(height: 10),
              pw.Table.fromTextArray(
                headers: _tableHeadersBC,
                data: _tableDataBC,
  ...
  Future<void> _savePdfToFirebaseStorage() async {
    ...
    final fileName = 'Bon de Commande-${DateTime.now().toString()}.pdf';
  ...
  Future<void> _saveDownloadUrlToFirestore() async {
    ...
    await _firestore.collection('BC').add({
      // 'downloadUrl': _downloadUrl,
                ...
                child: SafeArea(
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Header('Documents', 'Bon de Commande'),
                        SizedBox(
                          height: 60,
                        ),
...
//Table temporaire
                            ...
                            _isLoading2 ? Center(
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(content: Text("Enregistrement du bon de commande avec succès"),backgroundColor:AppColors.them,showCloseIcon: true,)
                              );
                              // Navigator.push(
                              //   context,
                              //   MaterialPageRoute(
                        ...
                        Center(
                          child: PrimaryText(
                            text: 'LISTE DES BONS DE COMMANDE DE  ${EntrpNom.toUpperCase()}', size: 18, fontWeight: FontWeight.w800, color: AppColors.them,),
                        ),
                        SizedBox(
                          height: 20,
                        ),
                          ...
                          children: [
                            SizedBox(
                              ...
                              child: StreamBuilder<QuerySnapshot>(
                                  stream: FirebaseFirestore.instance.collection('BC').orderBy('dateAjout', descending: true).snapshots(),
                                            ...
                                            DataCell(Row(mainAxisAlignment: MainAxisAlignment.center,
                                              children: [
                                                ...
                                                IconButton(
                                                    ...
                                                    icon: Icon(Icons.delete, color: Colors.red),
                                                    onPressed: () async {
                                                      await FirebaseFirestore.instance.collection('BC').doc(doc.id).delete();
