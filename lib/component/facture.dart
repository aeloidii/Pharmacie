...
class invoice extends StatefulWidget {
  ...
  @override
  State<invoice> createState() => invoiceState();
}
...
/*generate dmnd PDF*/
  ...
  Future<void> _generateInvoice() async {
    ...
    for (var item in listOfListsDesArticles.reversed){
      List<String> _list=[item[8],item[0],_DateDeCommandeTextController.text=="" ? DateFormat('yyyy-MM-dd   HH:mm').format(DateTime.now()).substring(0,10) : _DateDeCommandeTextController.text.substring(0,10),item[2],item[3],item[4],item[5],item[6]];
...
//Table temporaire
                        ...
                        Center(
                          child: PrimaryText(
                            text: 'LISTE DES FACTURES DE  ${EntrpNom.toUpperCase()}', size: 18, fontWeight: FontWeight.w800, color: AppColors.them,),
                        ),
                        SizedBox(
                          height: 20,
                        ),
                          ...
                          children: [
                            SizedBox(
                              ...
                              child: StreamBuilder<QuerySnapshot>(
                                  stream: FirebaseFirestore.instance.collection('facture').orderBy('dateAjout', descending: true).snapshots(),
                                                ...
                                                IconButton(
                                                    ...
                                                    onPressed: () {
                                                      Navigator.push(
                                                        context,
                                                        MaterialPageRoute(
                                                          builder: (context) => editInvoice(docId: routeArgs['id_Commande'],),
                                                        ),
                                                      );
                                                    }),
                                                IconButton(
                                                    ...
                                                    icon: Icon(Icons.delete, color: Colors.red),
                                                    onPressed: () async {
                                                      await FirebaseFirestore.instance.collection('facture').doc(doc.id).delete();
