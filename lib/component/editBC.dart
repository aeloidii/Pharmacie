...
class editBC extends StatefulWidget {
  String docId;
  editBC({super.key, required this.docId});

  @override
  State<editBC> createState() => editBCState();
}

class editBCState extends State<editBC> {
...
/*generate dmnd PDF*/
  ...
  Future<void> _saveDownloadUrlToFirestore() async {
    ...
    async {
      ...
      await _generateInvoice("Bon de Commande");
      await _savePdfToFirebaseStorage("Bon de Commande");
      firebaseFiretore
          .collection(
          "BC")
          .doc(widget.docId)
      ...
      for (int i = 0; i < listOfListsDesArticles.length; i++) {
       ...
       await _firestore
        ...
        if(statuRecoi=="Reçue"){
          print("referenceArticle ${listOfListsDesArticles[i][8]}");
          print("Quantit ${listOfListsDesArticles[i][2]}");
          await incrementerStock(listOfListsDesArticles[i][8],listOfListsDesArticles[i][2]);
        }
      }
  ...
  Future<void> _createInvoiceToFirestore() async { // STOCKER DANS FACTURE AVEC LE MEME ID DE LA COMMANDE
    setState(() {
      _isLoading2 = true;
    });
    await _firestore
  ...
  Future<void> _print() async {
    ...
    await _generateInvoice("Demande de Prix");
    final pdfBytes = await _pdf.save();
    await Printing.layoutPdf(
        onLayout: (PdfPageFormat format) async => pdfBytes);
    id_commande="";
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        PrimaryText(
                          text: 'Editer UN BON DE COMMANDE ',
                          size: 18,
                                  ...
                                  child:  Visibility(
                                    ...
                                    child: Stack(
                                      ...
                                      firebaseUIButton(
                                          ...
                                          // await _print();
                                          await _createInvoiceToFirestore();
                                          ...
                                          ScaffoldMessenger.of(context).showSnackBar(
                                              SnackBar(content: Text("Enregistrement du facture avec succès"),backgroundColor:AppColors.them,showCloseIcon: true,));                          // _desigFournTextController.text="";
                                        }, AppColors.btn, AppColors.them),
                                        Positioned(
                                          bottom: 30,
                                          right: 10,
