...
class editBL extends StatefulWidget {
  String docId;
  editBL({super.key, required this.docId});

  @override
  State<editBL> createState() => editBLState();
}

class editBLState extends State<editBL> {
...
/*generate dmnd PDF*/
      ...
      pw.Page(
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              statuPayment=="Payée" && whichDoc=="Facture" ? pw.Row(
  ...
  Future<void> _createBLToFirestore() async { // STOCKER DANS FACTURE AVEC LE MEME ID DE LA COMMANDE
    ...
    async {
      ...
      await FirebaseFirestore.instance.collection('BL').doc(widget.docId)
  ...
  Future<void> _addPAIEMToFirestore() async { // STOCKER DANS FACTURE AVEC LE MEME ID DE LA COMMANDE
...
/*generate dmnd PDF*/
  ...
  Future getDmndPrixFromDatabase() async {
    await FirebaseFirestore.instance
      ...
      if (snapshot.exists) {
        setState(() async {
            id_livreur=snapshot.data()!["id_livreur"];
            statuPayment=snapshot.data()!["etat"];
          ...
          await FirebaseFirestore.instance
            ...
            await FirebaseFirestore.instance
                ...
                .then((snapshot) async {
              if (snapshot.exists) {
                _desigLivreurTextController.text = await snapshot.data()!["nom"];
                ...
                child: SafeArea(
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Header('Documents', 'Edition du Bon de Livraison'),
                        SizedBox(
                          height: 60,
                        ),
                        Responsive.isDesktop(context)?
                        PrimaryText(
                          text: 'EDITER UN BON DE LIVRAISON ',
                          size: 18,
                                  ...
                                  child:  Visibility(
                                    ...
                                    child: Stack(
                                      ...
                                      firebaseUIButton(
                                        ...
                                        // await _print();
                                        await _addPAIEMToFirestore();


                                        ScaffoldMessenger.of(context).showSnackBar(
                                            SnackBar(content: Text("Enregistrement du paiement avec succès"),backgroundColor:AppColors.them,showCloseIcon: true,));                          // _desigFournTextController.text="";
                                      }, AppColors.btn, AppColors.them),
                                        Positioned(
                                          bottom: 30,
                                          right: 10,
